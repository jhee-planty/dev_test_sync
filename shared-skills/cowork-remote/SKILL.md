...
카테고리 기반 3-Strike와 별도로, 서비스별 `total_attempts` (전체 실패 건수)를 누적 추적한다.
- `total_attempts` ≥ 5 → genai-apf-pipeline §4b 서면 정당화 게이트 트리거
- 빌드 변경 시에도 리셋하지 않음 (카테고리 카운터와 다름)
- NEEDS_ALTERNATIVE 전환 시에만 리셋
- 근거: DuckDuckGo 17건, qianwen 12건 과다 투자 (4/17 회고)
