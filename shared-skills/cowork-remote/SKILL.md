...
### 1b. 정체 에스컬레이션 — 무응답 20분 시 다음 서비스로 진행

현재 서비스에서 결과가 도착하지 않을 때 무한 대기하지 않는다.
**2중 감지** 방식으로 세션 재시작에도 타임아웃이 작동한다.

**방법 1 — 카운트 기반 (활성 세션 내):**
- 빈 폴링(결과 없음) → `stall_count` +1
- 결과 도착 또는 서비스 변경 → `stall_count` 리셋 (0)
- **`stall_count` ≥ 4 (5분 간격 기준 약 20분):**
  → 에스컬레이션 동작 실행 (아래)

**방법 2 — 시간 기반 (세션 재시작 시에도 유효):**
- 가장 최근 request 파일의 타임스탬프 확인
- result 파일이 없고, request 생성 후 **20분 경과** 시:
  → 에스컬레이션 동작 실행 (아래)

**에스컬레이션 동작 (방법 1 또는 2 트리거 시):**
1. 현재 서비스를 `STALLED`로 표시
2. `monitoring.visual_needed = true` 설정 (L3 진단 대상)
3. service_queue에서 다음 `pending_check` 서비스로 진행
4. macOS 알림 전송: "{서비스} 20분 무응답 — 다음 서비스로 진행"
...
poll #3: 결과 없음 → stall_count=3
poll #4: 결과 없음 → stall_count=4 ≥ 4
  → Gemini를 STALLED로 표시
...
```
새 세션 시작 → requests/ 스캔
→ #370 request: 18:20 생성, result 없음, 현재 18:45 (25분 경과)
→ 즉시 에스컬레이션: "qwen3 #370 — 25분 무응답, STALLED"
```

**이 규칙이 없으면:** test PC 오프라인 시 14시간 무한 대기 발생 (4/10 회고 실측).
4회 연속 회고에서 지적된 핵심 비효율 (30분+ gap × 10건 = 5시간+ 유휴).

### 1c. 대기 중 생산적 전환 — 폴링 대기 시 분석 작업 수행

에스컬레이션 후 다음 test 요청을 보내기 전에, **dev PC 단독 수행 가능한 작업**을 먼저 처리한다.
순수 대기 시간을 생산적으로 활용한다.

**fallback 작업 목록 (우선순위순):**
1. 다음 서비스의 design doc 검토/업데이트 (Phase 2/5 준비)
2. 현재 서비스의 impl journal 정리 및 가설 정리
3. 다음 서비스의 SQL/C++ 템플릿 사전 준비
4. status.md 서비스 현황 업데이트
5. 회고 보고서 중간 메모 기록

**이 규칙이 없으면:** 에스컬레이션 후에도 "다음 test 요청을 보낼 서비스가 없으면" 유휴 상태 재발.
...
### Auto-Classify: Verdict→Status 자동 매핑 (2026-04-14 회고 반영)

결과 JSON의 verdict 값에 따라 서비스 status를 자동 갱신한다. 수동 업데이트 누락을 방지.

| verdict | → status | 추가 액션 |
|---------|----------|----------|
| `PASS` / `WARNING_SHOWN` | `VERIFIED` | impl journal 성공 기록 + 다음 서비스 진행 |
| `BLOCKED` / `BLOCK_CONFIRMED` | `BLOCK_VERIFIED` | Phase 4(frontend-inspect)로 전환 |
| `NOT_BLOCKED` | `TEST_FAIL` | 원인 분석 → apf-warning-impl 3-Strike 체크 |
| `NOT_RENDERED` | `TEST_FAIL` | 같은 카테고리 카운트 증가 → Same-Category Escalation 체크 |
| `PAGE_LOAD_BLOCKED` | `STALLED` | 인프라 문제 — test PC 네트워크/인증서 확인 |
| `AUTH_REQUIRED` | `NEEDS_USER_SESSION` | 사용자 세션 필요 알림 |
| `TIMEOUT` | `STALLED` | test PC 네트워크/브라우저 상태 확인 → 재시도 |
| `ERROR` | (변경 없음) | 에러 상세 로그 확인 후 재시도 |
| (unknown) | (변경 없음) | 미매핑 verdict — warning 로그 기록 후 수동 리뷰 |
...
**요청 전송 전 Pending Check (2026-04-14 회고 반영):**
push 전에 results/ 디렉토리를 확인하여 미완료(pending) 요청이 2건 이상이면 push를 보류한다.
test PC의 처리 속도를 초과하여 요청이 쌓이면 대기 시간이 급증한다 (4/13 실측: 11분간 8건 push → 평균 58.4분 대기).
```
# Pending check: requests/ 중 results/에 대응 파일이 없는 건수 확인
pending_count = (requests/ 파일 수) - (results/ 파일 수)
if pending_count >= 2: push 대기 (다음 결과가 git pull로 도착한 후 재시도)
```
...
→ See `references/scheduled-task-flow.md` for Scheduled Task 실행 흐름 (recovery scan, git sync, result processing, Auto-SUSPEND/Auto-Classify, dashboard/알림).
