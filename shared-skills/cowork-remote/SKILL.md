...
## Schema Validation (30차 work-process-canonical 통합)

> **Canonical schema**: `cowork-micro-skills/references/work-process-canonical/event-schema.md`
> **Validator**: `cowork-micro-skills/references/work-process-canonical/tooling/result-schema-validator.py`
> **Provenance**: 30차 discussion-review consensus (`discussions/2026-04-30_ai-work-process-normalization.md`)

### Archive-time validation (Step 3.d 보강)

§B (결과 폴링 + 처리) 의 step 3.d (`archive-completed.sh`) 직전, validator 호출:

```bash
# Pre-archive validation
VALIDATOR="$HOME/Documents/workspace/claude_work/projects/cowork-micro-skills/references/work-process-canonical/tooling/result-schema-validator.py"
python3 "$VALIDATOR" "$GIT_SYNC_REPO/results/{id}_result.json"
RC=$?

case $RC in
  0)  # OK — proceed to archive
      bash $RT_DIR/archive-completed.sh {id}
      ;;
  1)  # ERROR (Tier-1 missing) — reject + log
      echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) REJECT #{id}: Tier-1 missing" \
           >> "$GIT_SYNC_REPO/results/_validation_errors.log"
      # Do NOT archive. test-PC will see file remain in results/ → re-emit eligible.
      ;;
  2)  # WARN (Tier-2 missing for verdict-class) — log + archive
      echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) WARN #{id}: Tier-2 missing" \
           >> "$GIT_SYNC_REPO/results/_validation_warnings.log"
      bash $RT_DIR/archive-completed.sh {id}
      ;;
esac
```

**3-severity policy** (per work-process-canonical/event-schema.md):
- **ERROR (Tier-1 missing)**: reject, log to `results/_validation_errors.log`, file remains in results/ → test-PC re-emit
- **WARN (Tier-2 missing)**: accept + log to `results/_validation_warnings.log`, flag in cycle{N}-summary.md drift section
- **INFO (Tier-3 unknown field)**: accept silently, accumulate in `tooling/_unknown_fields.log` for next schema rev

### v0 grandfather

기존 343 historical result.json 은 `_schema_version: "v0"` 태그로 grandfather. Validator 가 v0 skip.

One-shot tag:
```bash
$HOME/Documents/workspace/claude_work/projects/cowork-micro-skills/references/work-process-canonical/tooling/result-schema-tag-v0.sh --apply
```

2026-04-30 forward 부터 emit 되는 result.json 은 v1 (validator 강제).

### Schema rev cadence

- Tier-3 → Tier-2 promotion: INFO field 가 last 3 months 동안 >30% 빈도 도달 시 discussion-review 제안.
- Tier-1 enum 변경 (overall_status 6-enum 등): discussion-review M3 + INTENTS §5 entry 필요.
- Breaking change: 새 vN, 기존 frozen.
