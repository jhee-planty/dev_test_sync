...
### Mode 2 — 자동 폴링 모드 (Scheduled Task 기반)
...
mcp__scheduled-tasks__create_scheduled_task({
  taskId: "dev-test-polling",       // 또는 "test-dev-polling"
  description: "dev_test_sync 폴링: 새 결과/요청 확인",
...
| GitHub MCP | ❌ | 인증 실패 (Bad credentials) |
| 파일 읽기/쓰기 | ✅ | desktop-commander 경유 |

**따라서 Scheduled Task에서는 git CLI를 desktop-commander로 실행한다:**
```
# GitHub MCP 대신 git CLI 사용
desktop-commander → git -C ~/Documents/workspace/dev_test_sync fetch origin
desktop-commander → git -C ~/Documents/workspace/dev_test_sync pull origin main
desktop-commander → ls ~/Documents/workspace/dev_test_sync/results/
desktop-commander → cat ~/Documents/workspace/dev_test_sync/results/{new_file}
```

**폴링 스킬 내용 (dev PC 기준):**
```
Scheduled Task 폴링 절차:
  1. git -C {GIT_SYNC_REPO} fetch origin
  2. git -C {GIT_SYNC_REPO} diff --name-only HEAD..origin/main -- results/
  3. 새 파일 없으면 → "No new results" 보고 후 종료
  4. 새 파일 있으면 → git pull → 파일 내용 읽기 → 결과 요약 보고
  5. state.json 업데이트 (last_checked_result_id)
```
...
Scheduled Task 폴링 절차:
  1. git -C {GIT_SYNC_REPO} fetch origin
  2. git -C {GIT_SYNC_REPO} diff --name-only HEAD..origin/main -- requests/
  3. 새 파일 없으면 → "No new requests" 보고 후 종료
  4. 새 파일 있으면 → git pull → 요청 읽기 → 작업 실행 → 결과 작성 → git push
```
...
```
  1. 역할 확인 (dev / test)
  2. 해당 스킬 로드 (dev: cowork-remote, test: test-pc-worker)
  3. state.json 읽어서 마지막 ID 확인
  4. git fetch/pull로 최신 상태 동기화
  5. Scheduled Task 생성 (cron 설정)
```
