...
| PC | 도구 | 비고 |
|----|------|------|
| dev (Cowork) | **GitHub MCP connector** (`mcp__github__*`) | `push_files`로 업로드, `get_file_contents`로 읽기, `list_commits`로 새 커밋 확인 |
| test (Windows) | **git CLI** via desktop-commander | `git fetch`, `git pull`, `git add`, `git commit`, `git push` |

**dev PC(Cowork)는 git CLI 대신 GitHub MCP connector를 사용한다.**
...
- dev: results/ 스캔 (git fetch) → 새 결과 도착 시 자동 queue 업데이트 + 보고
- test: requests/ 스캔 (git fetch) → 새 요청 도착 시 즉시 실행 + 결과 작성
