...
→ See also `genai-apf-pipeline` §Phase 3 결과 대기 → L3 시각 진단 for additional escalation (AnyDesk screenshot).
...
3. 새 빌드 배포 시 → 카테고리별 failure_history 리셋, SUSPENDED 해제

**총 투자 추적 (Total Attempts Counter):**
카테고리 기반 3-Strike와 별도로, 서비스별 `total_attempts`를 누적 추적한다.
→ See `genai-apf-pipeline` §4b (총 투자 상한 — 서면 정당화 게이트) for 정책 정의.
cowork-remote는 total_attempts 필드를 기록하고, 정책 판단은 pipeline이 수행한다.

**이 규칙이 없으면:** Mistral 동일 실패 10회(4/10), DuckDuckGo 17건(4/17) 과다 투자 반복

→ See `references/pipeline-state-schema.md` for `failure_history`, `total_attempts` 필드 스키마.
...
2. 현재 PC에 다른 APF 스킬(genai-apf-pipeline, etap-build-deploy 등)이
   설치되어 있으면 → **dev** (개발 환경)
...
- **`genai-apf-pipeline`**: Warning 구현 후 test PC에서 검증 요청 시 이 스킬을 통해 전달.
