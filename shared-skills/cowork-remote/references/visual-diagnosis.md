...
```
primary:    git polling (L1) — 항상 동작 (ScheduleWakeup chain 종결 절대 X)
secondary:  SSH etap 로그 (L2) — diagnostic 보조 (parallel, polling break 아님)
visual:     AnyDesk 스크린샷 (L3) — diagnostic 보조 (parallel, polling break 아님)
escalation: exclusive control — 사용자 명시적 요청 시에만

> **Non-applicability (★ D9 Stage 3 catch)**: L2/L3 의 "5분/30분" 표기는 information point only. polling chain termination trigger 아님. L1 polling 은 termination 2 조건 (결과 도착 / session 종료) 만 적용 — `genai-apf-pipeline/references/autonomous-execution-protocol.md §Termination Conditions` canonical.
```
