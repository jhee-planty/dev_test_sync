...
>
> WSA v3 primary filter (41차 mission-goal-based): `not startswith('defer:') AND not startswith('terminate:') AND not startswith('infra_blocked:')`. Primary 고갈 시 ≠ stop license — WSA v3 step 5-A~5-F expansion search 의무 (genai-apf-pipeline/SKILL.md §Work Selection Algorithm v3). infra_unblock_check:* 는 session 시작 시 자동 실행 후 일반 verb 로 전환.

> **`unverified_deploys` counter field** (2026-04-29 22차 추가, per-service entry) — `apply_engine_fix:*` deploy 가 verify 없이 누적된 횟수. 매 deploy 시 +1, 성공적 verify 시 reset to 0. **41차/45차 amendment**: 기존 "≥3 도달 시 자동 defer:awaiting_verification" hard count threshold 폐지 (count cap 일괄 cause-based 변환). 본 counter 는 **verification gap 가시화용 metric** 으로만 유지. 다음 deploy 전 evaluation: counter > 0 AND verification axis (test PC alive / UI verify path / regression suite) 가 동일 axis 로 fail 누적이면 axis pivot (D20(b) verify rotation, alternative verify method, sub-agent dispatch) — cause-based decision. count 자체로 자동 stop / defer 전환은 발생 안 함. D14(a) 호환 — `debug_*:*` 작업은 counter 영향 없음 (debug = output 인정).
...
> **`next_action_skill_pointer` + `next_action_references` fields** (2026-05-07 56차 discussion-review consensus, per-service entry, optional additive metadata — Form C) — micro-skill design 의 substantive implementation. LLM 이 entry read 시 자동 학습 trigger. context window 부담 minimize.
>
> **Schema**:
> ```json
> {
>   "service": "perplexity",
>   "next_action": "design_V7_G_perplexity_ask_SSE_target",
>   "next_action_skill_pointer": "apf-warning-impl",
>   "next_action_references": [
>     "services/perplexity_design.md",
>     "references/iteration-flow.md"
>   ]
> }
> ```
>
> **Backward compatibility**: 기존 string-form `next_action` 보존. 추가 fields 는 additive metadata — 없으면 legacy mode (기존 SKILL.md 의 fixed mapping 활용).
>
> **의무**:
> - `next_action_skill_pointer`: 다음 step 의 작업이 어느 skill 영역인지 명시 (e.g., `apf-warning-impl` / `etap-build-deploy` / `cowork-remote` / `test-pc-worker`)
> - `next_action_references`: 다음 step 작업 시 참조 의무 reference 들의 path list (relative to skill root, e.g., `services/{service}_design.md` / `references/phase{N}-*.md`)
> - LLM 이 entry read 시 → skill_pointer 의 SKILL.md re-read + references 학습 의무 (positive procedure D27 적용)
>
> **Cross-skill hand-off**: skill_pointer 가 다음 read 대상 명시 — wrapper invoke (`invoke-build-deploy.sh` 등) 후 skill_pointer 가 가리키는 skill 의 SKILL.md re-read 의무.
>
> **Anti-pattern (D9 Stage 5 호환)**: skill_pointer + references 만 채우고 실제 read 안 함 = Performative Compliance — 차단. 실제 read 의무.
>
> **Migration**: 점진적 적용. 1 entry test 후 user directive 시 모든 entries 적용 (다른 session 영향 큼).
>
> **Reference**: 56차 discussion-review consensus = INTENTS §5 56차 entry. canonical source = phase reference (e.g., `genai-apf-pipeline/references/phase6-warning-impl-orchestration.md`) — skill_pointer + references 가 phase reference 의 직접 pointer.
