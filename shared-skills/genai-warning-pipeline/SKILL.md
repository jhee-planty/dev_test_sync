...
description: >
  Use this skill when the user wants to manage or coordinate the APF Warning Pipeline itself — not perform a specific technical task within a phase. Invoke for: checking pipeline status across services, advancing to the next phase, onboarding a new service into the pipeline, reviewing cross-service warning progress, or understanding the overall workflow. The key signal: the user is thinking about the pipeline as a whole rather than executing a concrete step. Do NOT invoke when the user asks to actually perform hands-on work like capturing network traffic, inspecting a frontend, writing code, or deploying — those belong to phase-specific sub-skills. Trigger on pipeline-management language ("pipeline 상태", "다음 phase", "새 서비스 추가", "전체 현황"); skip for action-oriented task language aimed at a specific technical operation.
---
...
돕릭 컨텍스트가 필요하다. 이 물리적 제약이 역할 분담을 결정한다.
...
각 서비스의 진행상태. 새 서비스 추�/ 상태 변경 시 업데이트한다.
...
- Phase 3 → Phase 2: design이 실제 렌더링과 닠를 때 (재설계)
...
핌스트 요청은 순차적으로 보낸되, 결과 분석을 병렬화하는 것이 핵심이다.
...
4. 모든 분석 완료 → 종합 판정
   ...
   → 확인 불가(자동화 거부, 페이지 민로딩) → 해당 서비스 제외, 나머지 진행
```
...
턄서 대기 시간을 병렬화하여 전체 사이클을 단축하는 것이다.
...
blocked=1이 오판인 경우:
  - 프론트엔드 도메인의 페이지 로드 요청읈 차단됨 (프롬프트 API는 무관)
  - path_patterns='/' → 정적 리소스, 분서 요청까지 매칭
...
- **컨텍스트 관리**: 50~100턴니다 /compact, 대용량 데이터는 파일 참조
