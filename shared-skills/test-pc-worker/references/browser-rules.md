...
```
입력 전략 에스컬레이션 (각 strategy 가 root cause 명확히 fail 하면 다음 strategy 로 — count cap 아닌 cause-based, 41차):
  1. SendKeys (기본)
...
```
/compact 자율 트리거 불가 (test-pc-worker SKILL.md §3 user-preferences canonical). 본 섹션 historical only:
  - 같은 root cause 가 새 axis 시도 후에도 재현 → result.json push + 다음 request 진행
  - 스크린샷 동일 + 결과 다름 기대 = anti-pattern, 즉시 strategy pivot
