...
각 command별 windows-mcp + PowerShell 실행법.
요청 JSON의 `command`를 확인하고 아래 절차대로 실행한다.

**핵심 도구:** `mcp__windows-mcp__PowerShell` 로 PowerShell 명령을 실행한다.
파일 I/O 는 `mcp__windows-mcp__FileSystem` (read / write / list / create) 를 사용한다.
Chrome 자동화 GUI 상호작용은 `mcp__windows-mcp__Click` / `Type` / `Screenshot`.

**Legacy**: 2026-04-23 11차 이전 protocol 은 `mcp__desktop-commander__*`. desktop-commander 가 설치된 환경에서는 동등 기능으로 대체 가능 (`start_process ↔ PowerShell`, `list_directory/read_file/write_file ↔ FileSystem`).
...
`mcp__windows-mcp__FileSystem` 로 폴더 스캔 (list) + 각 JSON 읽기 (read).
PowerShell 스크립트는 `mcp__windows-mcp__PowerShell` 로 직접 실행.
...
3. **로그인 확인** — 스크린샷으로 로그인 필요 여부 판단. 필요 시 result.json `notes` 에 "login required: {service}" 기록 (test PC user channel 부재, push = dev side 통지).
...
2. **steps가 있으면** 각 단계 순차 실행:
   - `"login"` → status="error_AUTH_REQUIRED" 결과 push 후 step 종료, 다음 step skip (HR3 — chain wait 금지)
...
windows-mcp (또는 legacy desktop-commander) 호출마다 네트워크 왕복이 발생한다. 호출 횟수를 줄이면 전체 처리 속도가 크게 향상된다.
