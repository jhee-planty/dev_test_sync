...
description: "Use this skill when the user is in hands-on implementation mode for an APF warning - actively writing, fixing, or testing code for a specific service. Trigger on: C++ generator function work, warning rendering problems, HTTP/2 block response behavior, is_http2 field values, test log management, false positives, or blocked=1 diagnostics. The goal is making a warning actually work or verifying it works, not deciding on an approach. Do NOT trigger for design documents, architecture/strategy comparisons, frontend DOM inspection, or HAR analysis - those belong to other skills."
---

# APF Warning Implementation Skill

## Quick Reference (매 iteration 확인)
- **성공 기준:** 경고 문구가 브라우저에 표시되면 PASS → See `genai-apf-pipeline` §서비스별 목표 기준 for 상세
...
- **Implementation Entry Check:** design doc strategy ↔ impl is_http2 일치 확인
...
→ See `../genai-apf-pipeline/SKILL.md` § "API 엔드포인트 파악 방법"
...
genai-apf-pipeline에서 Phase 3 진입 시 자동으로 로드되지만,
context break 후 직접 이 스킬로 작업을 재개할 때는 수동 로드가 필요하다.

### Implementation Entry Check (design↔impl 일관성)
...
**리그레션 테스트 절차:**
1. `../genai-apf-pipeline/SKILL.md`의 Service Status 테이블에서 VERIFIED/DONE 상태인 서비스 목록 확인
...
**한 번에 한 서비스만 작업한다.** (3/25 회고: 3개 동시 7빌드 → 성공 0건. 단일 서비스 집중이 효율적.)

```
순차 실행 흐름:
  1. genai-apf-pipeline의 우선순위 테이블에서 다음 서비스 선택
...
→ See `../genai-apf-pipeline/SKILL.md` → "서비스 우선순위" 테이블
...
- **`genai-apf-pipeline`**: Master orchestrator — triggers this skill for Phase 3.
