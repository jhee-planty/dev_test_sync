...
**서비스당 전체 상한: 유효 7회 빌드-테스트 사이클. (하드 게이트)**
유효 카운트 7회 소진 시 어떤 방식이 남아있더라도 BLOCKED_ONLY로 처리한다.

**유효 카운트 규칙 — 실패 유형별 차등 적용:**

| 유형 | 게이트 카운트 | 판정 기준 |
|------|------------|----------|
| tweakable | **카운트** | 응답 형식 조정 (JSON 키, SSE 구분자, is_http2 값 등) |
| structural | **카운트** (1회 즉시 전환) | 프론트엔드가 에러를 catch, 자체 UI 표시 |
| code_bug | **면제** (서비스당 최대 1회) | 시스템 수준 코드 결함. 사전 분류 + impl journal 증거 필수 |
| infra_issue | **면제** | DB 설정 오류, DPDK 재시작 실패 등 코드와 무관한 인프라 문제 |
| external_change | **카운트 리셋** | 서비스 프론트엔드가 업데이트되어 이전 시도 무효화 |

→ See `references/escalation-protocol.md` for 유형별 상세 판정 기준.

**code_bug 면제 조건 (3가지 모두 충족):**
1. 버그의 원인이 코드 수준에서 특정됨 (파일명 + 함수명 + 라인)
2. 수정 방안이 구체적으로 기술됨 (변경 내용 + 영향 범위)
3. 서비스별 응답 형식 조정이 아닌 **시스템 수준 결함** (다른 서비스에도 영향 가능)
면제는 **빌드 전에 사전 분류**하고 impl journal에 기록해야 한다.
빌드 후 사후 주장은 불가.

**강제 승인 게이트:** 유효 5회 소진 시점에서 반드시 사용자에게 현황을 보고하고
추가 빌드 승인을 받는다. "현재 {서비스} 유효 5/7빌드 소진. 시도한 방식: {목록}.
남은 2빌드로 계속하시겠습니까, BLOCKED_ONLY로 판정하시겠습니까?"
사용자가 "계속"이라 해도 유효 7회 도달 시 추가 승인 없이 BLOCKED_ONLY로 확정한다.
...
**infra_issue 반복 에스컬레이션:** 동일 유형의 인프라 문제가 3회 이상 발생하면
warning pipeline이 아닌 인프라 팀에 에스컬레이션한다.
