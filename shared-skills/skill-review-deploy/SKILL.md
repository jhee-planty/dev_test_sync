...
description: >
  ...
  Do NOT trigger for multi-participant discussion-based review — that belongs to skill-discussion-review.
...
**환경 주의:** Cowork 마운트(`/sessions/.../mnt/.claude/skills/`)는 read-only이고
캐시된 내용이 stale할 수 있다. 반드시 host 경로에서 읽는다.
...
# 2. Git commit & push
cd "$SHARED/.." && git add -A && git commit -m "Skill review: ..." && git push
```
