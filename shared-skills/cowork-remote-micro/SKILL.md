---
name: cowork-remote-micro
description: Dev PC (Mac) ↔ Test PC (Windows) 간 Git 기반 비동기 협업. dev 가 request push, test 가 result push. 본 skill 은 dev 쪽 전담 (test 쪽은 test-pc-worker-micro). Use when user says "test PC 에 요청 보내줘", "check-warning {서비스}", "check-block {서비스}", "큐 상태", "결과 확인", "원격 작업", "폴링", "result scan", or any cross-PC coordination. 결정론적 runtime (bash scripts) 으로 git/filesystem 작업 수행, Claude 는 자연어→command 추론 + 결과 성공/실패 판정만 담당. Auto STALLED 에스컬레이션 없음 (응답 도착까지 반복 scan). 3-Strike 는 genai-apf-pipeline-micro 에서 판정.
...
- ❌ Scheduled Task / auto-polling mode (MEMORY.md §13.4 No schedulers)
...
- `test-pc-worker-micro` : test PC 쪽 pair.
- `etap-build-deploy-micro` : 빌드/배포 완료 후 본 skill 로 검증 요청 push.
- `apf-warning-impl-micro` : warning impl 세션 중 본 skill 통해 test 왕복.
- `genai-apf-pipeline-micro` : 최상위 orchestrator. 본 skill 을 반복 호출.
