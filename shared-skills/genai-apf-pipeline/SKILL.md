...
- **status.md는 현재 수동 유지** (regen-status.sh 미구현 — `services/status.md` Maintenance Note 참조)
...
status.md는 **현재 수동 유지** 방식이다. `regen-status.sh`는 미구현 상태 (2026-04-14 확인).
주기적 `apf_pipeline_report_YYYYMMDD.md` 작성 후 해당 분류를 status.md에 복사하는
방식으로 유지한다. 상세는 `services/status.md` Maintenance Note 참조.
(TODO: regen-status.sh 작성 시 impl journal → status.md 자동 재생성으로 전환)
...
```
context break 재개 흐름:
  0. services/status.md 읽기 (수동 유지 — regen-status.sh 미구현)
...
| Pipeline status | all | `services/status.md` (수동 유지 — regen-status.sh 미구현) |
