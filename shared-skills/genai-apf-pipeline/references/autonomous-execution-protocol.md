...
5. **idle 대기 금지** — "알림을 기다린다"며 멈추지 않는다. **목표 미달성 상태 = 항상 다음 해법 탐색 + 시도**. 새 hypothesis brainstorm / 다른 angle 시도 / 재시도 / spec design / paper work 등 dev-side autonomous activity 가 default. fabricated work 는 회피하되, 진정한 "할 일 없음" 은 mission goal 달성 시점에만 가능. *(Enforced by: Stop hook architectural primary; Self-Check Cat E cognitive backup)*
...
7. **Mission-Goal Persistence (41차 amendment — count-based stop logic 폐지)** — 본 rule 의 stop 자격 = **mission goal achieved** 만. count-based stop ("autonomous_candidates count==0") 폐지 (41차 사용자 directive: "목표를 달성하기 전까지 계속 해법을 찾고 시도하는 작업을 반복").
   - **(7-1) Stop Hook (D16(a), 22차 — 41차 amendment)**: Claude 가 추가 tool 호출 없이 응답 종료 시도 시 `.claude/hooks/stop-autonomous-guard.sh` 자동 fire. **mission goal 미달성 (DONE/TOTAL < 1.0) AND 사용자 메시지에 termination keyword 없음 → stop block + 재engagement 강제**. count 무관.
   - **목표 미달성 시 expansion search 의무**: dev-side blocked items 만 있어도 stop X. 다음 시도 source:
     - 새 hypothesis brainstorm (existing diagnosis 재검토)
     - Strategy revisit (A/B/C/D/E 다른 strategy)
     - Paper work (spec design / schema extension / reference cleanup / tooling)
     - Sub-agent dispatch 가능한 분석/검토 task
     - Reference doc 보강 / lessons append
   - **Subagent in-flight** 이면 stop 금지 (subagent return 까지 대기 — 19차 보강).
   - **M4 overgeneralization 차단** (22차 cycle95 incident): user-required task (M4) 가 일부 있어도 dev-side autonomous task 가 mission 진전에 기여 가능하면 진행. 일부 task 의 M4 가 전체 stop 정당화 X.
   - **Honest Idle = mission goal achieved (DONE/TOTAL = 1.0)** 시점만. 그 외 모든 stop = HR5 violation.
...
4. **Duration cap 인지** — 1 polling 목적당 max 6hrs 또는 `expected_result_at + 30min` 중 작은 값.
   ...
   - **Non-applicability (자명한 negative space)**: termination trigger 로 사용 **금지**. polling 행동 영향 없음. 본 cap 도달 시 chain 유지 + 사용자에게 상태 보고 (보고 = continuation, 종료 아님). Termination 은 `§Termination Conditions` (위 섹션) 의 2 조건만 (결과 도착 / session 종료). 본 항목 인용해서 timeout-based self-termination 도출 = D9 anti-pattern Stage 3 (deontic citation).
...
> **Termination 은 `§Termination Conditions` 의 2 조건만**: (1) 결과 도착, (2) session 종료. 그 외 모든 self-termination 금지.
...
본 카테고리 자기 점검 (41차 — count-based 폐지, mission-goal-based 로 재정의):

- **Summary 작성했는가?** → 다음 행동 = "next push" 여야 함. doc-write-then-stop pattern 차단.
- **"완료" 선언?** → 권한 = 사용자만. mission goal (DONE / TOTAL = 1.0) 미달성 시 자기 종료 결정 금지.
- **Stop 시도?** → mission goal 미달성 = expansion search (WSA v3 step 5-A~5-F) 의무. count 무관.
- **"Diminishing returns" / "어려운 작업" 으로 자기 정지?** → DONE 카운트 변화 없어도 cumulative learning = 진전. 새 hypothesis / strategy revisit / paper work 항상 가능. 사용자 직접 halt 만 termination.
- **자기 imposed 제약 ("no more probes" / "HIGH RISK 분류" / "5-strike streak" 등) 사용자 directive 와 충돌?** → directive 우선 (anchoring 방지). Self-imposed risk label 은 Cat G violation.
...
2. < 1.0 시: WSA v3 step 5 expansion search source (5-A~5-F) 중 1개 선택 + 즉시 시도

**D9 Anti-Pattern Stage List (canonical, evolution chronology)**:

| Stage | Form | Codify | 표지어 (telltale) |
|-------|------|--------|------------------|
| 1 | Hard count cap | 21차 (3-strike auto-suspend), 41차 폐지 | "3-strike 도달", "5/7 빌드 cap", "5-iter 소진" |
...
- **Summary / status / progress doc 작성 후 stop 시도?** → Stop hook 가 reminder 발동. mission goal 미달성 시 doc 작성 자체는 OK, **doc-write-then-stop pattern 차단**. count 0 주장도 무효.
- **M4 (user-required) encounter 발견?** → 해당 task 만 `defer:*`/`infra_blocked:*`, 다른 candidate 영향 없음. **M4 overgeneralization 차단** (cycle95 incident).
- **Long-running session 끝 즈음 reasoning 약화 감지?** → mission goal 미달성이면 expansion search 계속. fatigue stop bias 차단.
- **"User-side action 필요" 만 보고 stop?** → dev-side autonomous activity (paper work / sub-agent / lesson harvest) 동시 가능 여부 점검 의무. user-blocked items 만 보고 dev-side 누락 = 41차 trigger 패턴.
...
## Honest Idle Protocol (D19(b), 25차 → 41차 redefined)

**Honest Idle = mission goal achieved 시점만**. 41차 amendment: count-based stop 폐지, idle 정의를 mission-goal 기반으로 재정의.

### 2-state hierarchy (41차 simplified)

```
State A: Mission goal 미달성 (DONE / (TOTAL - TERMINAL_UNREACHABLE) < 1.0)
         → STOP 금지. WSA v3 step 5 expansion search (5-A~5-F) 적용.
         → Primary candidate exhausted 라도 새 hypothesis / strategy revisit / sub-agent / paper work / lesson harvest / D20b verify 등 항상 시도 가능.

State B: Mission goal achieved (DONE / TOTAL = 1.0) + 30-day D20b PASS 누적
         → Maintenance mode 전환. Honest idle 정당화.
```

> **Note**: 25차 의 4-state hierarchy (State 1-4) 는 41차 폐지. 특히 "State 2 (no candidates but blockers)" 와 "State 4 (truly idle)" 가 stop license 로 남용되던 패턴 차단.

### Mission goal 미달성 시 expansion search (의무)

primary candidate 가 모두 defer/terminate/infra_blocked 더라도 다음 시도 source **항상** 존재:

| Source | 예시 |
|--------|------|
| New hypothesis brainstorm | BLOCKED_diagnosed 의 cause_pointer 재검토, 다른 angle 탐색 |
| Strategy revisit | A/B/C/D/E 다른 strategy 시도, envelope schema rev |
| Sub-agent dispatch | HAR audit, log mining, code review, design analysis |
| Paper work | spec design, schema extension, reference cleanup, tooling |
| Lesson harvest | failure_history 패턴 분석 → INTENTS / lessons append |
| D20(b) verification | DONE services L1 canary + L2-2A UI verify rotation |

### 적용 원칙

- **Stop license 의 유일 source = mission goal**. count / blocker enumeration 은 stop 근거 아님.
- **Fabrication 회피**: 위 expansion source 도 provenance 검증 (Cat I D19(a)) 필요. 자기 재미 위한 work 금지.
- **사용자 explicit termination keyword** (정지/종료/그만/halt/quit 등) 만 manual stop 허용.
