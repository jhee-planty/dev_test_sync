...
If any early determination condition in Section 3.3 is met, skip Steps 3-5 and use the NEEDS_ALTERNATIVE output format with justification.
...
> Phase 3 진입 전에 NEEDS_ALTERNATIVE를 판정하고, design doc에 근거를 명시한다.
...
| 에러 처리 | 에러 핸들러 범위, 에러 UI 유형, 에러 역할 대체 가능성, silent failure | 대안 방법 트리거 |
...
| 조기 판정 조건(3.3)을 확인했는가 | 해당 시 NEEDS_ALTERNATIVE 판정과 근거가 명시되었는지 |
...
조기 판정(Section 3.3)으로 NEEDS_ALTERNATIVE가 확정된 서비스용 단축 템플릿.
...
### Strategy
- Pattern: NEEDS_ALTERNATIVE (대안 접근법 시도 필요)
- 근거: {조기 판정 조건과 구체적 근거}
