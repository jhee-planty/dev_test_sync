...
| 3 | block-verify | etap-build-deploy-micro 호출, cowork-remote-micro check-block 왕복 | 화면+로그 ground truth 판정, BLOCK_ONLY gate |
| 4 | frontend-inspect | cowork-remote-micro 로 test PC 에서 DOM profile 수집 | delivery_method 결정 (http_api/websocket/sse/grpc) |
| 5 | warning-design | design sub-agent dispatch, design doc 초안 저장 | strategy A/B/C/D 선택, is_http2 값 결정 |
| 6 | warning-impl | **apf-warning-impl-micro 위임** (iteration loop) | (apf-warning-impl-micro 가 담당) |
| 7 | release-build | **etap-build-deploy-micro 위임** | post-build verify 판정 |
...
|----------|-----------|----------------|
| Phase 3, 4, 5 의 test PC 왕복 | cowork-remote-micro | `runtime/cowork-remote/push-request.sh` 직접 호출 |
| Phase 3 빌드 | etap-build-deploy-micro | `runtime/etap-build-deploy/etap-build-deploy.sh` 직접 호출 |
| Phase 6 전체 | apf-warning-impl-micro | `runtime/apf-warning-impl/record-iteration.sh` + gate + Claude |
| Phase 7 전체 | etap-build-deploy-micro | `runtime/etap-build-deploy/etap-build-deploy.sh` |
...
| 총 빌드 상한 (apf-warning-impl-micro 와 독립) | runtime `enforce-3strike.sh` | build 10 초과 시 서비스 HOLD |
...
- 질문으로 끝맺지 않음
- 결과 대기 중 STALLED 자동 전환 없음 (cowork-remote-micro 가 scan 반복)
...
- `cowork-remote-micro`, `test-pc-worker-micro`, `etap-build-deploy-micro`, `apf-warning-impl-micro` — 본 skill 이 wrapper 경유 호출.
