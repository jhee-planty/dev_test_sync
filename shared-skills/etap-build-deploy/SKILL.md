...
1. **빌드 실패 진단** — step 2 ninja_build 실패 시 log 자율 분석 → 코드 수정 직접 적용 → 재빌드. 3회 연속 같은 step 실패 시에만 ESCALATE.
...
3. **post-install anomaly** — step 8 tail 에러 패턴 → 분류 + 자율 수정 시도 (config / restart). dependency 결손 시 ESCALATE.
