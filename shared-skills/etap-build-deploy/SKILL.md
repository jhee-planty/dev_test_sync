...
**필수 검증: 페이지 정상 로드 확인 (page-load 차단 방지)**

APF 코드를 포함한 빌드 배포 후, 키워드 테스트 전에 반드시 서비스 페이지의 정상 로드를 확인한다.

```
배포 후 page-load 검증 체크리스트:
  [필수] 키워드 없이 서비스 페이지 접속 → 정상 로드 확인
         (서비스 메인 페이지가 정상적으로 표시되는지)
  [선택] 일반 대화 흐름 확인 (키워드 미포함 프롬프트 → 정상 응답)
         (이전에 page-load 차단이 발생했던 서비스에서만 필수)
  [이후] 키워드 포함 프롬프트로 차단 동작 확인
```

> **왜 이 검증이 필요한가:** 2026-04-10 deepseek, gemini에서 페이지 접속 자체가
> 차단되는 page-load 즉시 차단이 발생. APF는 프롬프트 기반 필터이므로
> 페이지 접속 차단은 절대 발생해서는 안 되는 동작이다.
> 사용자 피드백: "서비스 제공 시 절대 하지 말아야 하는 동작."
