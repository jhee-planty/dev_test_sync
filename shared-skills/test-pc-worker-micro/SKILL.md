...
# test-pc-worker-micro

Test PC (Windows) 전용 micro-control skill. Pair of `cowork-remote-micro` (dev 쪽).
...
- ❌ Heartbeat.json (cowork-remote-micro 가 scan timing 을 제어하므로 불필요)
...
- `cowork-remote-micro` (dev 쪽 pair) — request/result 교환 프로토콜.
- `genai-apf-pipeline-micro` — 최상위 orchestrator (dev 쪽).
