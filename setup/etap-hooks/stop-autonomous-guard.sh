...
try:
    ...
    # 56차 amendment: skill_pointer + references additive display (LLM 자동 학습 trigger)
    for e in queue[:5]:
        st = e.get('status', '?')
        na = e.get('next_action', '')
        svc = e.get('service', '?')
        sp = e.get('next_action_skill_pointer', '')
        refs = e.get('next_action_references', [])
        line = f"  {svc}: status={st} next_action={na}"
        if sp:
            line += f" [skill={sp}"
            if refs:
