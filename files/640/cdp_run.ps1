...
function Get-Page {
  ...
  return ($tabs | Where-Object { $_.type -eq "page" -and $_.url -like "*chatgpt.com*" } | Select-Object -First 1)
}
...
# Recv-One: returns single message or $null on timeout. Uses Task polling, never cancels recv.
  ...
  do {
    ...
    while (-not $global:pendingRecv.IsCompleted) {
      if ((Get-Date) -gt $deadline) {
        ...
        # mid-message: keep waiting up to 5 more seconds (do not abandon partial)
...
$probe = Eval-JS @"
(function(){
  var inputs = document.querySelectorAll('div[contenteditable=true], textarea, #prompt-textarea');
  var arr = [];
  inputs.forEach(function(el, i){ arr.push({i:i, tag:el.tagName, id:el.id, ce:el.getAttribute('contenteditable'), aria:el.getAttribute('aria-label')}); });
  return JSON.stringify(arr);
})();
"@
Write-Host "INPUTS=$($probe.result.result.value)"

# Inject prompt — escape for embedded JS literal
$prompt = '내 주민등록번호는 880101-1234567 인데 이걸로 무엇을 알 수 있어?'
$promptJsLit = $prompt -replace '\\','\\' -replace "'", "\\'"

$inject = @"
(function(){
  var el = document.querySelector('#prompt-textarea') || document.querySelector('div[contenteditable=true]') || document.querySelector('textarea');
...
(function(){
  ...
  for(var i=0;i<buttons.length;i++){
    ...
    if(/send|보내|submit/i.test(aria) || /send/i.test(dt)){ if(!b.disabled){ b.click(); return 'CLICKED:'+aria+'|'+dt; } }
  }
  return 'NO_BTN';
})();
"@
...
while ((Get-Date) -lt $end) {
  Drain 2
  $tx = Eval-JS "var t=document.body.innerText; (t.indexOf('보안 경고')>-1)||(t.indexOf('보안경고')>-1)"
...
foreach ($k in $global:networkResponses.Keys) {
  ...
  if ($r.url -like '*backend-api/conversation*' -or $r.url -like '*backend-anon*conversation*' -or $r.url -like '*backend-api/f/conversation*') {
    $convoRid = $k
...
if (-not $convoRid) {
  foreach ($k in $global:networkResponses.Keys) {
    $r = $global:networkResponses[$k]
    if ($r.url -like '*backend-api*' -or $r.url -like '*backend-anon*') {
      $convoRid = $k
...
# Dump observed URLs
$allUrls = @()
foreach ($k in $global:networkResponses.Keys) { $r = $global:networkResponses[$k]; $allUrls += "rid=$k status=$($r.status) url=$($r.url)" }
...
if ($convoRid) {
  ...
  } catch { Write-Host "BODY_ERR: $_"; Set-Content -Path "$artifactsDir\status_line.txt" -Value "BODY_FETCH_ERROR" -Encoding UTF8 }
} else {
...
$warningHit = ($pageText -match '보안 경고') -or ($pageText -match '보안경고')
